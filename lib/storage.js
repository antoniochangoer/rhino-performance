...
export async function getSharedWith(programId) {
  ...
  const { data, error } = await supabase
    .from("program_shares")
    .select("shared_with, status, profiles!program_shares_shared_with_fkey(username, email)")
    .eq("program_id", programId);
...
export async function getPendingShareRequests() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      status,
      created_at,
      programs ( id, name, description, goal, total_weeks ),
      profiles!program_shares_program_id_fkey ( username, email )
    `)
  ...
  return (data || []).map((row) => ({
    ...
    fromUsername: row.profiles?.username || "—",
    fromEmail: row.profiles?.email || "",
...
export async function getGymPartners() {
  ...
  // People I shared programs with (accepted) + people who shared programs with me (accepted)
  const { data: sharedByMe } = await supabase
    .from("program_shares")
    .select("shared_with, status, profiles!program_shares_shared_with_fkey(username, email), programs!inner(name)")
    .eq("programs.user_id", user.id)
    .eq("status", "accepted");

  const { data: sharedWithMe } = await supabase
    .from("program_shares")
    .select("status, programs!inner(id, name, user_id), profiles!program_shares_program_id_fkey(username, email)")
    .eq("shared_with", user.id)
    .eq("status", "accepted");
