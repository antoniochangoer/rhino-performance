...
export async function acceptShareRequest(programId) {
  ...
  // Try to fetch the source program (RLS allows this because a pending share row exists)
  // Use maybeSingle to avoid 406 if the row is somehow inaccessible
  ...
  // Deep-copy sessions + exercises (only if we could read them)
  if (sourceProgram?.sessions?.length > 0) {
    const sessions = (sourceProgram.sessions).sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
    for (const session of sessions) {
      ...
      const exercises = (session.exercises || []).sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
