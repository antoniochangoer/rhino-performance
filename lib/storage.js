...
import { getMaxHR, getKarvonenZones, ageFromBirthYear } from "./hrZones";
...
export async function getPrograms() {
  ...
  // Get user's active_program_id from profile
  ...
  // Find accepted share rows where we are the recipient and a copy was already made
  // — these original programs should be hidden (we show the copy instead)
  const { data: acceptedShares } = await supabase
    .from("program_shares")
    .select("program_id, copy_program_id")
    .eq("shared_with", user.id)
    .eq("status", "accepted");

  // Set of original program IDs that already have a copy owned by us — hide the originals
  const alreadyCopiedOriginalIds = new Set(
    (acceptedShares || [])
      .filter((s) => s.copy_program_id)
      .map((s) => s.program_id)
  );

  return (programs || [])
    .filter((p) => {
      // Hide the owner's original if we already have a copy
      if (p.user_id !== user.id && alreadyCopiedOriginalIds.has(p.id)) return false;
      return true;
    })
    .map((p) => {
      const normalized = normalizeProgram(p);
      normalized.active = p.id === activeProgramId;
      // A copied share is owned by user but has shared_from set
      if (p.shared_from) {
        normalized.isShared = true;
        normalized.ownerUsername = p.shared_from_name || "—";
      } else {
        normalized.isShared = false;
        normalized.ownerUsername = null;
      }
      return normalized;
    });
}
...
export async function setActiveProgram(id) {
  ...
  // Store active program per user in profiles (works for own + shared programs)
  await supabase
    .from("profiles")
...
export async function getActiveProgram() {
  ...
  // Get active_program_id from profile
  const { data: profile } = await supabase
    .from("profiles")
    .select("active_program_id")
    .eq("id", user.id)
  ...
  if (!profile?.active_program_id) return null;
...
export async function updateExercise(programId, sessionId, exerciseId, updates) {
  ...
  // If 1RM changed, patch the active training log JSONB so RPE colors stay accurate
  if (updates.oneRepMax !== undefined) {
    const activeLog = await getActiveLog();
    if (activeLog && activeLog.programId === programId) {
      const updatedExercises = activeLog.exercises.map((ex) =>
        ex.exerciseId === exerciseId
          ? { ...ex, oneRepMax: updates.oneRepMax }
          : ex,
      );
      await supabase
        .from("training_logs")
...
export async function getAllExerciseNames() {
  ...
  if (!error) {
    ...
    console.error("getAllExerciseNames:", error);
  }

  // From active program (own or shared copy) so receiver sees same exercises as sender
...
export async function shareProgram(programId, emailOrUsername) {
  ...
  // Fetch program name to store in share row (so recipient can read it without programs RLS)
  const { data: prog } = await supabase
    .from("programs")
    .select("name, goal, total_weeks")
    .eq("id", programId)
    .single();

  // Insert with status "pending" — recipient must accept
  const { error } = await supabase
    .from("program_shares")
    .insert({
      program_id: programId,
      shared_with: found.id,
      status: "pending",
      owner_id: user.id,
      program_name: prog?.name || "",
      program_goal: prog?.goal || "",
      program_weeks: prog?.total_weeks || null,
    });
...
export async function getSharedWith(programId) {
  ...
  // Fetch profile info per user separately to avoid FK join ambiguity
  const result = [];
  for (const row of data || []) {
    const { data: profile } = await supabase
      .from("profiles")
      .select("username, email")
      .eq("id", row.shared_with)
      .maybeSingle();
    result.push({
      id: row.shared_with,
...
export async function getPendingShareRequests() {
  ...
  const { data, error } = await supabase
    .from("program_shares")
    .select("program_id, status, created_at, program_name, program_goal, program_weeks, owner_id")
    .eq("shared_with", user.id)
    .eq("status", "pending");

  if (error) { console.error("getPendingShareRequests:", error); return []; }

  console.log("getPendingShareRequests raw data:", JSON.stringify(data));

  // Fetch owner profiles (profiles table is publicly readable)
  ...
  return (data || []).map((row) => {
    ...
    return {
      ...
      programName: row.program_name || "Onbekend schema",
      programGoal: row.program_goal,
      programWeeks: row.program_weeks,
...
export async function acceptShareRequest(programId) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;

  // Run the deep-copy server-side via SECURITY DEFINER RPC
  // This bypasses all RLS issues for reading owner's sessions/exercises
  const { data, error } = await supabase.rpc("accept_share_and_copy", {
    p_program_id: programId,
    p_recipient_id: user.id,
  });

  if (error) {
    console.error("acceptShareRequest RPC failed:", error);
  } else {
    console.log("acceptShareRequest: copy created, new program id:", data);
  }
}

export async function declineShareRequest(programId) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;

  // If this is already accepted, programId may be the COPY's program id (owned by user).
  // Check if this program is a copy (shared_from is set) and delete it too.
  const { data: prog } = await supabase
    .from("programs")
  ...
  if (prog && prog.user_id === user.id && prog.shared_from) {
    // Delete the program copy — cascades to sessions + exercises
    await supabase.from("programs").delete().eq("id", programId);
    // Clean up the share row — try both copy_program_id and owner_id lookups
    const { error: e1 } = await supabase
      .from("program_shares")
      .delete()
      .eq("copy_program_id", programId)
      .eq("shared_with", user.id);
    if (e1) {
      // Fallback: delete by owner_id match
      await supabase
        .from("program_shares")
        .delete()
        .eq("owner_id", prog.shared_from)
        .eq("shared_with", user.id);
    }
    return;
  }

  // Pending decline — just remove the share row
  const { error } = await supabase
    .from("program_shares")
    .delete()
    .eq("program_id", programId)
  ...
  if (error) console.error("declineShareRequest:", error);
}

// ---- Main Lifts 1RM ----

export async function saveMainLifts1RM(lifts) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;

  // Always persist to localStorage as reliable fallback
  try { localStorage.setItem(`main_lifts_1rm_${user.id}`, JSON.stringify(lifts)); } catch (_) {}

  const { error } = await supabase
    .from("profiles")
    .update({ main_lifts_1rm: lifts })
    .eq("id", user.id);

  if (error) console.error("saveMainLifts1RM (DB):", error);
}

export async function getMainLifts1RM() {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return {};
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("main_lifts_1rm")
    .eq("id", user.id)
    .single();

  if (!error && data?.main_lifts_1rm && Object.keys(data.main_lifts_1rm).length > 0) {
    return data.main_lifts_1rm;
  }

  // Fall back to localStorage (works even if the DB column doesn't exist yet)
  try {
    const stored = localStorage.getItem(`main_lifts_1rm_${user.id}`);
...
export async function getPartnerExerciseHistory(exerciseName) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];

  // Fetch all own programs to find share chains (both as owner and as recipient)
  ...
  // Collect all partner log rows across all programs
  const allRows = [];
  for (const prog of programs) {
    const { data: rows, error } = await supabase.rpc("get_partner_logs", {
      p_program_id: prog.id,
      p_user_id: user.id,
    });
    if (error) { console.error("getPartnerExerciseHistory RPC:", error); continue; }
    if (rows && rows.length > 0) allRows.push(...rows);
  }

  if (allRows.length === 0) return [];

  // Group by partner and extract e1RM for the selected exercise
  const byPartner = {};
  for (const row of allRows) {
    const pid = row.partner_id;
    if (!byPartner[pid]) byPartner[pid] = { username: row.partner_name, entries: [] };
    ...
    if (bestE1rm > 0) {
      byPartner[pid].entries.push({ date: row.log_date, e1rm: bestE1rm, bestWeight });
    }
  }

  return Object.entries(byPartner).map(([partnerId, { username, entries }]) => {
    const byDate = {};
    for (const r of entries) {
      if (!byDate[r.date] || r.e1rm > byDate[r.date].e1rm) byDate[r.date] = r;
    }
    return {
      partnerId,
      username,
...
export async function updateProfileHR({ birthYear, restingHeartRate }) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: "Niet ingelogd" };

  const updates = {};
  if (birthYear !== undefined) updates.birth_year = birthYear ? Number(birthYear) : null;
  if (restingHeartRate !== undefined) updates.resting_heart_rate = restingHeartRate ? Number(restingHeartRate) : null;
...
export async function getCardioActivities(fromDate, toDate) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];
  ...
  if (error) {
    console.error("getCardioActivities:", error);
    return [];
  }
  return (data || []).map((row) => ({
    id: row.id,
    ...
    type: row.type || "aerobic",
    zone: row.zone ?? null,
    avgHr: row.avg_hr ?? null,
    maxHr: row.max_hr ?? null,
    notes: row.notes || "",
    rounds: row.rounds ?? null,
    workIntervalMin: row.work_interval_min ?? null,
    restIntervalMin: row.rest_interval_min ?? null,
    warmupMin: row.warmup_min ?? null,
    cooldownMin: row.cooldown_min ?? null,
    createdAt: row.created_at,
  }));
}

function computeDurationFromRounds(rounds, workIntervalMin, restIntervalMin, warmupMin, cooldownMin) {
  const r = Number(rounds) || 0;
  const w = Number(workIntervalMin) || 0;
  const rest = Number(restIntervalMin) || 0;
  const warm = Number(warmupMin) || 0;
  const cool = Number(cooldownMin) || 0;
  if (r < 1 || (w === 0 && rest === 0)) return null;
  return warm + r * (w + rest) + cool;
}

/** Resolve zone and type from profile Karvonen zones when HR is given. Source of truth = profile zones. */
async function resolveZoneAndTypeFromProfile(avgHr, maxHr) {
  const profile = await getProfile();
  if (!profile) return { zone: null, type: null };
  const age = ageFromBirthYear(profile.birth_year);
  const maxHR = age != null ? getMaxHR(age) : null;
  if (maxHR == null) return { zone: null, type: null };
  const restHR = profile.resting_heart_rate ?? 70;
  const zones = getKarvonenZones(restHR, maxHR);
  if (zones.length === 0) return { zone: null, type: null };
  const hr = Number(avgHr ?? maxHr);
  if (!hr || hr < 40 || hr > 250) return { zone: null, type: null };
  const z = zones.find((zone) => hr >= zone.bpmMin && hr <= zone.bpmMax);
  if (!z) return { zone: null, type: null };
  return { zone: z.zone, type: z.type };
}

export async function createCardioActivity({
  date, name, durationMinutes, type, zone, avgHr, maxHr, notes,
  rounds, workIntervalMin, restIntervalMin, warmupMin, cooldownMin,
}) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;

  const computed = computeDurationFromRounds(rounds, workIntervalMin, restIntervalMin, warmupMin, cooldownMin);
  const duration = computed !== null ? computed : (Number(durationMinutes) || 0);

  let resolvedZone = zone >= 1 && zone <= 5 ? zone : null;
  let resolvedType = type === "anaerobic" ? "anaerobic" : "aerobic";
  if (avgHr != null && avgHr !== "" || maxHr != null && maxHr !== "") {
    const fromProfile = await resolveZoneAndTypeFromProfile(avgHr ? Number(avgHr) : null, maxHr ? Number(maxHr) : null);
    if (fromProfile.zone != null) {
      resolvedZone = fromProfile.zone;
      resolvedType = fromProfile.type || resolvedType;
    }
  }
  ...
  if (rounds != null && rounds !== "") insert.rounds = Number(rounds) || null;
  if (workIntervalMin != null && workIntervalMin !== "") insert.work_interval_min = Number(workIntervalMin) || null;
  if (restIntervalMin != null && restIntervalMin !== "") insert.rest_interval_min = Number(restIntervalMin) || null;
  if (warmupMin != null && warmupMin !== "") insert.warmup_min = Number(warmupMin) || null;
  if (cooldownMin != null && cooldownMin !== "") insert.cooldown_min = Number(cooldownMin) || null;
  ...
  if (error) {
    console.error("createCardioActivity:", error);
    return null;
  }
  return {
    id: data.id,
    ...
    workIntervalMin: data.work_interval_min ?? null,
    restIntervalMin: data.rest_interval_min ?? null,
    warmupMin: data.warmup_min ?? null,
    cooldownMin: data.cooldown_min ?? null,
    createdAt: data.created_at,
  };
}

export async function updateCardioActivity(id, updates) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;
  ...
  if (updates.name !== undefined) dbUpdates.name = updates.name;
  if (updates.durationMinutes !== undefined) dbUpdates.duration_minutes = Number(updates.durationMinutes);
  if (updates.avgHr !== undefined) dbUpdates.avg_hr = updates.avgHr ? Number(updates.avgHr) : null;
  if (updates.maxHr !== undefined) dbUpdates.max_hr = updates.maxHr ? Number(updates.maxHr) : null;
  if (updates.notes !== undefined) dbUpdates.notes = updates.notes;
  if (updates.rounds !== undefined) dbUpdates.rounds = updates.rounds ? Number(updates.rounds) : null;
  if (updates.workIntervalMin !== undefined) dbUpdates.work_interval_min = updates.workIntervalMin ? Number(updates.workIntervalMin) : null;
  if (updates.restIntervalMin !== undefined) dbUpdates.rest_interval_min = updates.restIntervalMin ? Number(updates.restIntervalMin) : null;
  if (updates.warmupMin !== undefined) dbUpdates.warmup_min = updates.warmupMin ? Number(updates.warmupMin) : null;
  if (updates.cooldownMin !== undefined) dbUpdates.cooldown_min = updates.cooldownMin ? Number(updates.cooldownMin) : null;

  if (updates.avgHr !== undefined || updates.maxHr !== undefined) {
    const fromProfile = await resolveZoneAndTypeFromProfile(dbUpdates.avg_hr ?? null, dbUpdates.max_hr ?? null);
    if (fromProfile.zone != null) {
      dbUpdates.zone = fromProfile.zone;
      dbUpdates.type = fromProfile.type || "aerobic";
    }
  } else {
    if (updates.type !== undefined) dbUpdates.type = updates.type === "anaerobic" ? "anaerobic" : "aerobic";
    if (updates.zone !== undefined) dbUpdates.zone = updates.zone >= 1 && updates.zone <= 5 ? updates.zone : null;
  }

  await supabase.from("cardio_activities").update(dbUpdates).eq("id", id).eq("user_id", user.id);
}

export async function deleteCardioActivity(id) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;
  await supabase.from("cardio_activities").delete().eq("id", id).eq("user_id", user.id);
}
...
export async function getGymPartners() {
  ...
  // Programs I own that I shared with others (accepted)
  ...
  const myProgramMap = Object.fromEntries((myPrograms || []).map((p) => [p.id, p.name]));

  const { data: sharedByMe } = myProgramIds.length
    ? await supabase
        .from("program_shares")
        .select("program_id, shared_with")
        .in("program_id", myProgramIds)
        .eq("status", "accepted")
    : { data: [] };

  // Programs shared with me (accepted)
  const { data: sharedWithMe } = await supabase
    .from("program_shares")
    .select("program_id, programs ( id, name, user_id )")
    .eq("shared_with", user.id)
    .eq("status", "accepted");
  ...
  for (const row of sharedByMe || []) partnerIds.add(row.shared_with);
  for (const row of sharedWithMe || []) {
    if (row.programs?.user_id) partnerIds.add(row.programs.user_id);
  }
  ...
  for (const row of sharedByMe || []) {
    ...
    if (!partnersMap[id]) {
      ...
      partnersMap[id] = { id, username: p.username || "—", email: p.email || "", sharedByMe: [], sharedWithMe: [] };
    }
    const name = myProgramMap[row.program_id];
    if (name) partnersMap[id].sharedByMe.push(name);
  }
  ...
  for (const row of sharedWithMe || []) {
    ...
    if (!partnersMap[ownerId]) {
      ...
      partnersMap[ownerId] = { id: ownerId, username: p.username || "—", email: p.email || "", sharedByMe: [], sharedWithMe: [] };
    }
...
function normalizeProgram(p) {
  ...
  return {
    ...
    sharedFrom: p.shared_from || null,
    sharedFromName: p.shared_from_name || null,
    sessions,
  };
}
