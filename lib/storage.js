...
export async function getPrograms() {
  ...
  return (programs || []).map((p) => {
    ...
    const info = shareInfoMap[p.id];
    if (info) {
      normalized.isShared = true;
      normalized.ownerUsername = info.ownerUsername;
    } else {
      normalized.isShared = p.user_id !== user.id;
...
export async function acceptShareRequest(programId) {
  ...
  if (error) console.error("acceptShareRequest:", error);
}
...
export async function getPartnerLogsForProgram(programId) {
  ...
  // Find all partners linked to this program (bidirectional)
  const { data: shares } = await supabase
    .from("program_shares")
  ...
  // Collect partner user IDs (people who are not the current user)
  const partnerIds = new Set();
  for (const s of shares) {
    if (s.shared_with !== user.id) partnerIds.add(s.shared_with);
    if (s.owner_id && s.owner_id !== user.id) partnerIds.add(s.owner_id);
  }
  ...
  // Fetch partner logs for this program (requires RLS policy allowing this)
  ...
  if (error) { console.error("getPartnerLogsForProgram:", error); return []; }

  return (logs || []).map((l) => ({
    ...normalizeLog(l),
    partnerUsername: profileMap[l.user_id]?.username || profileMap[l.user_id]?.email || "Partner",
    partnerId: l.user_id,
  }));
}
