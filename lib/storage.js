...
export async function shareProgram(programId, emailOrUsername) {
  ...
  let sharedWithId = profile?.id;

  // If no match by username, try email via admin (not available client-side), so we fallback
  if (!sharedWithId) {
    return { error: "Gebruiker niet gevonden. Zorg dat ze een account hebben en geef hun gebruikersnaam op." };
  }

  const { error } = await supabase
    .from("program_shares")
    .insert({ program_id: programId, shared_with: sharedWithId });
...
export async function getSharedWith(programId) {
  ...
  const { data, error } = await supabase
    .from("program_shares")
    .select("shared_with, profiles!program_shares_shared_with_fkey(username)")
    .eq("program_id", programId);

  if (error) { console.error("getSharedWith:", error); return []; }
  return (data || []).map((row) => ({
    id: row.shared_with,
    username: row.profiles?.username || row.shared_with,
  }));
}
