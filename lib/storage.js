...
export async function acceptShareRequest(programId) {
  ...
  // Deep-copy the program + sessions + exercises so the recipient owns their own version
  // and is not affected if the owner deletes the original later.
  const { data: sourceProgram, error: progErr } = await supabase
    .from("programs")
    .select(`*, sessions (*, exercises (*))`)
    .eq("id", programId)
    .single();

  if (progErr || !sourceProgram) {
    console.error("acceptShareRequest: could not fetch source program", progErr);
    return;
  }

  // Get owner username for labelling
  const { data: ownerProfile } = await supabase
    .from("profiles")
    .select("username, email")
    .eq("id", sourceProgram.user_id)
    .maybeSingle();
  const ownerName = ownerProfile?.username || ownerProfile?.email || "—";
  ...
  const { data: newProgram, error: newProgErr } = await supabase
    ...
    .insert({
      ...
      goal: sourceProgram.goal || "peaking",
      total_weeks: sourceProgram.total_weeks || 8,
      start_rpe: sourceProgram.start_rpe || 7,
      current_index: sourceProgram.current_index || 0,
      current_week: sourceProgram.current_week || 1,
      active: false,
      shared_from: sourceProgram.user_id,
  ...
  // Mark share as accepted (keep row so partner-progress links still work)
  await supabase
    .from("program_shares")
