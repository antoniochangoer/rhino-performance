...
export async function acceptShareRequest(programId) {
  ...
  // Read the share row first — recipient always has SELECT on own share rows
  const { data: shareRow } = await supabase
    .from("program_shares")
    .select("program_id, owner_id, program_name, program_goal, program_weeks, status")
    .eq("program_id", programId)
    .eq("shared_with", user.id)
    .maybeSingle();

  if (!shareRow) {
    console.error("acceptShareRequest: share row not found");
    return;
  }

  // Fetch the source program, sessions and exercises in separate queries
  // (nested PostgREST joins can silently return empty arrays when RLS blocks child rows)
  ...
  if (sessionIds.length > 0) {
    ...
    for (const ex of allExercises || []) {
      if (!sourceExercisesBySession[ex.session_id]) sourceExercisesBySession[ex.session_id] = [];
      sourceExercisesBySession[ex.session_id].push(ex);
    }
  }

  // Get owner username from profiles (publicly readable)
  const ownerId = shareRow.owner_id || sourceProgram?.user_id;
  let ownerName = "—";
  if (ownerId) {
    const { data: ownerProfile } = await supabase
      ...
      .eq("id", ownerId)
      .maybeSingle();
    ownerName = ownerProfile?.username || ownerProfile?.email || "—";
  }

  // Build program data: prefer live data, fall back to what was stored in the share row
  const programName = sourceProgram?.name || shareRow.program_name || "Gedeeld schema";
  const programDescription = sourceProgram?.description || "";
  const programGoal = sourceProgram?.goal || shareRow.program_goal || "peaking";
  const programWeeks = sourceProgram?.total_weeks || shareRow.program_weeks || 8;
  const programStartRpe = sourceProgram?.start_rpe || 7;
  const programCurrentIndex = sourceProgram?.current_index || 0;
  const programCurrentWeek = sourceProgram?.current_week || 1;

  // Insert new program owned by the recipient
  const { data: newProgram, error: newProgErr } = await supabase
    .from("programs")
    .insert({
      user_id: user.id,
      name: programName,
      ...
      goal: programGoal,
      total_weeks: programWeeks,
      start_rpe: programStartRpe,
      current_index: programCurrentIndex,
      current_week: programCurrentWeek,
      active: false,
      shared_from: ownerId || null,
      shared_from_name: ownerName,
    })
    .select()
    .single();

  if (newProgErr || !newProgram) {
    console.error("acceptShareRequest: could not create program copy", newProgErr);
    return;
  }

  // Deep-copy sessions + exercises
  for (const session of sourceSessions || []) {
    const { data: newSession, error: sessErr } = await supabase
      .from("sessions")
      .insert({ program_id: newProgram.id, name: session.name, position: session.position ?? 0 })
      .select()
      .single();

    if (sessErr || !newSession) { console.error("acceptShareRequest: session copy failed", sessErr); continue; }

    const exercises = sourceExercisesBySession[session.id] || [];
    if (exercises.length > 0) {
      await supabase.from("exercises").insert(
        exercises.map((ex) => ({
          ...
          start_rpe: ex.start_rpe || 7,
          one_rep_max: ex.one_rep_max || 0,
  ...
  // Mark share as accepted and store the copy's program id
  await supabase
    .from("program_shares")
    .update({ status: "accepted", copy_program_id: newProgram.id })
    .eq("program_id", programId)
    .eq("shared_with", user.id);
}
...
export async function getPartnerExerciseHistory(exerciseName, programId) {
  const logs = await getPartnerLogsForProgram(programId);
  if (!logs.length) return [];

  // Group by partner
  const byPartner = {};
  for (const log of logs) {
    const pid = log.partnerId;
    if (!byPartner[pid]) byPartner[pid] = { username: log.partnerUsername, entries: [] };
    ...
    if (bestE1rm > 0) {
      byPartner[pid].entries.push({ date: log.date, e1rm: bestE1rm, bestWeight });
    }
  }
...
export async function getPartnerLogsForProgram(programId) {
  const supabase = getSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];

  // Fetch the program to know who the owner is
  const { data: prog } = await supabase
    .from("programs")
    .select("user_id, shared_from")
    .eq("id", programId)
    .maybeSingle();

  if (!prog) return [];

  const iAmOwner = prog.user_id === user.id;
  const partnerLogSets = []; // { userId, programId }

  if (iAmOwner) {
    // I own the original — find all accepted shares and their copy program IDs
    const { data: shares } = await supabase
      .from("program_shares")
      .select("shared_with, copy_program_id")
      .eq("program_id", programId)
      .eq("status", "accepted");

    for (const s of shares || []) {
      if (s.copy_program_id) {
        partnerLogSets.push({ userId: s.shared_with, programId: s.copy_program_id });
      }
    }
  } else if (prog.shared_from) {
    // I am a recipient — find the original program owned by shared_from
    // and also find other recipients of the same original
    const { data: shares } = await supabase
      .from("program_shares")
      .select("shared_with, owner_id, copy_program_id")
      .eq("status", "accepted");

    // Find the share row that created my copy to learn the original program_id
    const myShare = (shares || []).find((s) => s.shared_with === user.id && s.copy_program_id === programId);
    if (!myShare) return [];

    const originalProgramId = myShare ? null : null; // we need the original program_id
    // Get original program_id via owner's programs
    const { data: ownerProgs } = await supabase
      .from("programs")
      .select("id")
      .eq("user_id", prog.shared_from)
      .eq("name", prog.name); // best-effort match

    const originalId = ownerProgs?.[0]?.id;
    if (originalId) {
      // Owner's own logs
      partnerLogSets.push({ userId: prog.shared_from, programId: originalId });

      // Other recipients' copies
      const otherShares = (shares || []).filter(
        (s) => s.owner_id === prog.shared_from && s.shared_with !== user.id && s.copy_program_id,
      );
  ...
  if (partnerLogSets.length === 0) return [];

  // Collect unique partner user IDs for profile fetch
  const partnerIds = [...new Set(partnerLogSets.map((s) => s.userId))];
  const { data: profiles } = await supabase
    .from("profiles")
    .select("id, username, email")
    .in("id", partnerIds);
  ...
  // Fetch logs for each partner's program copy
  const allLogs = [];
  for (const { userId, programId: pid } of partnerLogSets) {
    ...
    for (const l of logs || []) {
      allLogs.push({
        ...normalizeLog(l),
        partnerUsername: profileMap[userId]?.username || profileMap[userId]?.email || "Partner",
        partnerId: userId,
      });
    }
  }
