...
export async function shareProgram(programId, emailOrUsername) {
  ...
  const { error } = await supabase
    .from("program_shares")
    .insert({ program_id: programId, shared_with: found.id, status: "pending" });
...
export async function getPendingShareRequests() {
  ...
  const { data, error } = await supabase
    .from("program_shares")
    .select("program_id, status, created_at, programs ( id, name, goal, total_weeks, user_id )")
    .eq("shared_with", user.id)
    .eq("status", "pending");

  if (error) { console.error("getPendingShareRequests:", error); return []; }

  // Fetch owner profile per row to avoid FK ambiguity
  const result = [];
  for (const row of data || []) {
    ...
    if (ownerId) {
      const { data: ownerProfile } = await supabase
        ...
        .eq("id", ownerId)
        .maybeSingle();
      fromUsername = ownerProfile?.username || "—";
      fromEmail = ownerProfile?.email || "";
    }
    result.push({
      programId: row.program_id,
      programName: row.programs?.name || "Onbekend schema",
      programGoal: row.programs?.goal,
      programWeeks: row.programs?.total_weeks,
      fromUsername,
      fromEmail,
