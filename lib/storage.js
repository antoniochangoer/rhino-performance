...
export async function getPrograms() {
  ...
  // Fetch share info for programs we don't own (to show owner name)
  const sharedProgramIds = (programs || [])
  ...
  if (sharedProgramIds.length > 0) {
    const { data: shares } = await supabase
      .from("program_shares")
      .select("program_id, owner_id, program_name")
      .in("program_id", sharedProgramIds)
      .eq("shared_with", user.id)
      .eq("status", "accepted");

    const ownerIds = [...new Set((shares || []).map((s) => s.owner_id).filter(Boolean))];
    const profileMap = {};
    if (ownerIds.length > 0) {
      const { data: ownerProfiles } = await supabase
        ...
        .in("id", ownerIds);
      for (const p of ownerProfiles || []) profileMap[p.id] = p;
    }

    for (const s of shares || []) {
      const owner = profileMap[s.owner_id] || {};
      shareInfoMap[s.program_id] = {
        isShared: true,
